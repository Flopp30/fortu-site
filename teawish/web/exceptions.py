...
def exception_user_not_found_template_handler(request: Request, exc: Exception, templates: Jinja2Templates):
    ...
    response: HTMLResponse = templates.TemplateResponse(
        'components/refresh_page_content.html', {'request': request}, status_code=302
    )
...
def setup_exception_handlers(app: FastAPI, templates: Jinja2Templates):
    ...
    app.add_exception_handler(
        UserDoesNotExistsException, partial(exception_user_not_found_template_handler, templates=templates)
    )
    app.add_exception_handler(
        ExpiredSessionException, partial(exception_user_not_found_template_handler, templates=templates)
    )
